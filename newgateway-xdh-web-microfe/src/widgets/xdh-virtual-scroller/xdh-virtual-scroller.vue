<script>
  ...
  export default {
     ...
     * @property {Number} [itemHeight=null] 以像素为单位的内容项，用于计算滚动高度和位置。当为null的时候，会使用可变高度模式（Variable height mode）。
     * @property {Number|String} [minItemHeight=null] 如果内容项的高度未知，则使用最小高度。当mode为dynamic时，该字段为必填。
     ...
     * @property {String} [heightField='height'] 在Variable height mode下，获取内容项的高度的配置字段。官网建议最好为'height'
     ...
     */
    props: {
      ...RecycleScroller.mixins[0].props,
      mode: {
        type: String,
        validator(val) {
    ...
    computed: {
      scrollerProps() {
        ...
        // 官网要求dynamic滚动时，minItemHeight是必填项
        if (props.mode === 'dynamic' && !props.minItemHeight) {
          console.warn('minItemHeight is required')
          return false
        }
    ...
    render(h) {
      return h(
        ...
        [
          h('template', {slot: 'before-container'}, this.$slots['before-container']),
          h('template', {slot: 'after-container'}, this.$slots['after-container'])
        ]
      )
    }
  }
